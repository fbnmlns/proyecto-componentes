<svelte:head>
	<title>SvelteKit authentication with Firebase</title>
</svelte:head>

<h1 class="text-4xl font-bold">SvelteKit authentication with Firebase</h1>

<p>
	Blog post: <a
		href="https://www.okupter.com/blog/client-side-authentication-firebase-sveltekit"
		class="font-bold hover:text-blue-700 hover:underline"
		target="_blank">https://www.okupter.com/blog/client-side-authentication-firebase-sveltekit</a
	>
</p>

<p>
	GitHub repository: <a
		href="https://github.com/JustinyAhin/okupter-repos/tree/main/apps/sveltekit-auth-firebase"
		class="font-bold hover:text-blue-700 hover:underline"
		target="_blank"
		>https://github.com/JustinyAhin/okupter-repos/tree/main/apps/sveltekit-auth-firebase</a
	>
</p>